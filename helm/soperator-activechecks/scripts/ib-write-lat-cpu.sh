...
echo "Running ib_write_lat_cpu check on $(hostname)..."
HC_OUTPUT=$(srun --cpu-bind=verbose,cores bash -c "health-checker run -e soperator -p $platform -n ib_write_lat --json-log")
HC_EXIT_CODE=$?
