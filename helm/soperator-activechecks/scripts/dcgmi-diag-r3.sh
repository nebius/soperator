...
HC_OUTPUT=$(srun sudo bash -l -c "health-checker run -e soperator-acceptance -p $platform -i dcgmi_diag_r3 -f json-partial --tests-stdout-path /opt/soperator-outputs/local/health_checker_cmd_stdout --log-level info")
