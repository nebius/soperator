...
echo "Upgrading nc-health-checker to the version {{ .Values.checks.upgradeHealthChecker.healthCheckerVersion }}"
...
retry -d 2 -t 10 -- ssh -i /mnt/jail/opt/soperator-home/soperatorchecks/.ssh/soperatorchecks_id_ecdsa \
    ...
    'flock /var/lock/apt.lock bash -c "sudo apt update && sudo apt install --only-upgrade nc-health-checker={{ .Values.checks.upgradeHealthChecker.healthCheckerVersion }}"'
