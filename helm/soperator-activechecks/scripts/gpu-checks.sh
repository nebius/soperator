...
HC_OUTPUT=$(srun sudo bash -l -c "health-checker run -e soperator_active_checks -p $platform -f json-partial --tests-stdout-path /opt/soperator-outputs/local/health_checker_cmd_stdout --log-level info")
