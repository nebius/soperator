...
if [ "${rc}" -ne 0 ]; then
    ...
    ls -la "/usr/lib/$(uname -m)-linux-gnu/" | awk '/libnvidia-ml|libcuda\.so/ {print "[nvml]   " $0; n++} END {if (!n) print "[nvml]   no libnvidia-ml/libcuda entries"}'
    echo "[nvml] nvidia entries in ld.so.cache: $(ldconfig -p | grep -c nvidia)"
fi
exit "${rc}"
