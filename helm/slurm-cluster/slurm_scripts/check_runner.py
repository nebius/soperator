...
# Ensure Python has sufficient version
...
# Set up logging
try:
  log_path = os.environ.get("CHECKS_RUNNER_OUTPUT", "/dev/stdout")
...
class Check(typing.NamedTuple):
  ...
  # What platforms this check should run on.
  # Supported values:
  ...
  # In what contexts this check should run in.
  # Supported values:
  # - "any" - any context
...
try:
  ...
  SLURM_JOB_COMMENT = os.environ.get("SLURM_JOB_COMMENT", "")
...
def format_time(dt: datetime.datetime) -> string:
...
def get_gpu_platform_tags() -> typing.List[str]:
  try:
    res = subprocess.run(
...
def run_check(check: Check, in_jail=False):
  ...
  if result.returncode == 0:
    ...
    if check.on_ok == "undrain" and "DRAIN" in node_info().state_flags:
      if node_info().reason and node_info().reason.startswith(reason_base):
        undrain_node()
    elif check.on_ok == "uncomment":
      if node_info().comment and node_info().comment.startswith(reason_base):
        uncomment_node()
  else:
...
def node_info() -> NodeInfo:
  try:
    ...
    node = sinfo[0]
    info = NodeInfo(
      state_flags=node.get("node", {}).get("state", []),
