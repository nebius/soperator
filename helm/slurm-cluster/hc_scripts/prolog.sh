...
if [ -n "$SLURM_JOB_GPUS" ]; then
    ...
    chroot ${JAIL_DIR} /bin/bash -s <<-'EOF'
        ...
        details=$(health-checker run -e soperator -p {{ .Values.healthCheckScripts.platform }} -f mk8s-txt \
            -n {{ .Values.healthCheckScripts.prolog.checks }} 2>$log) || exit_code=$?
        printf "\n%s\n" "$details" >> "$log"
...
/mnt/jail/opt/soperator/custom-scripts/cleanup_enroot.sh  > "$log" 2>&1 || true

echo "Map the Slurm job with DCGM metrics"
log="${JAIL_DIR}${LOGS_OUTPUT_DIR}/${SLURMD_NODENAME}.map_job_dcgm.${SCRIPT_CONTEXT}.out"
/mnt/jail/opt/soperator/custom-scripts/map_job_dcgm.sh > "$log" 2>&1 || true
