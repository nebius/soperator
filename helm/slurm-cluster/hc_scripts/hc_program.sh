...
chroot /mnt/jail /bin/bash -s <<-'EOF'
    ...
    details=$(health-checker run -e soperator -p {{ .Values.healthCheckScripts.platform }} -f mk8s-txt \
        -n {{ .Values.healthCheckScripts.hcProgram.checks }} 2>$log) || exit_code=$?
    printf "\n%s\n" "$details" >> "$log"

    if [[ $exit_code -eq 0 ]]; then
        echo "Check ${check}: PASS"
    elif [[ $exit_code -eq 1 ]]; then
        echo "health-checker: FAIL with exit code 1 (${details})"
        ...
        echo "health-checker: FAIL with exit code 2 ${details}"
        exit 0
    fi
EOF
