...
class TestFlatOnlyWorker(unittest.TestCase):
    """A CPU-only worker belongs to a flat topology, which lists no nodes.

    Waiting for its hostname to appear in the config would never succeed, so such a worker waits
    only for the file and skips registration — but must still be undrained.
    """

    def test_bindings_are_flat_only(self):
        self.assertTrue(worker_init.bindings_are_flat_only([("cpu", "flat")]))
        self.assertFalse(worker_init.bindings_are_flat_only([("cpu", "flat"), ("ib", "tree")]))
        self.assertFalse(worker_init.bindings_are_flat_only([]))

    @mock.patch("worker_init.apply_node_topology")
    @mock.patch("worker_init.wait_for_topology_file")
    @mock.patch("worker_init.wait_for_hostname_in_topology_conf")
    def test_flat_worker_skips_the_hostname_wait(
        self, mock_wait_hostname, mock_wait_file, mock_apply
    ):
        env = {"HOSTNAME": "cpu-0", "SLURM_TOPOLOGY_BINDINGS": "cpu=flat"}
    ...
    @mock.patch("worker_init.get_node_addr", return_value="nodeaddr=cpu-0.svc")
    @mock.patch("subprocess.run")
    def test_empty_topology_still_undrains(self, mock_run, mock_get_node_addr):
        mock_run.return_value = mock.Mock(returncode=0, stdout="", stderr="")

        worker_init.apply_node_topology("cpu-0", "")
    ...
    def test_flat_bindings_contribute_no_units(self):
        self.assertEqual(
            worker_init.build_bound_topology('{"tier-0":"b1"}', [("cpu", "flat")], "root"), ""
        )
        self.assertEqual(
            worker_init.build_bound_unknown_topology([("cpu", "flat")], "root"), ""
        )
