...
def apply_node_topology(hostname: str, topology: str) -> None:
    ...
    try:
        ...
        # A flat topology defines no units, so there is nothing to register into. The rest of the
        # command still has to run: it clears a manual drain placed on a powered-down worker.
        if topology:
...
def bindings_are_flat_only(bindings: list[tuple[str, str]]) -> bool:
    """Return True when every topology covering this node is flat."""
    return bool(bindings) and all(kind == TOPOLOGY_TYPE_FLAT for _, kind in bindings)
...
def wait_for_topology_file(wait_timeout: int, poll_interval: int) -> Path:
    ...
    start_time: float = time.monotonic()
    while True:
        path: Path = resolve_topology_config_path()
        try:
...
def wait_for_topology() -> None:
    ...
    if bindings_are_flat_only(bindings):
        # A flat topology lists no nodes, so this worker will never appear in the config and has
        # no unit to register into.
        logger.info(
            "Node %s belongs only to flat topologies, skipping topology registration", hostname
        )
        wait_for_topology_file(wait_timeout, poll_interval)
