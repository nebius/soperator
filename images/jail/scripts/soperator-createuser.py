...
def run(cmd, *, passthrough=False, **kwargs):
  ...
  try:
    ...
    print(f"Error running: {' '.join(shlex.quote(str(c)) for c in cmd)}", file=sys.stderr)
...
def parse_args():
  ...
  p.add_argument("--with-password", action="store_true",
                 help="Prompt for a password")
  p.add_argument("--with-gecos", action="store_true",
                 help="Prompt for GECOS")
  p.add_argument("--without-sudo", action="store_true",
                 help="Do not add the user to the 'sudo' group")
  p.add_argument("--without-docker", action="store_true",
                 help="Do not add the user to the 'docker' group")
  p.add_argument("--without-internal-ssh", action="store_true",
                 help="Do not generate an internal SSH keypair (ECDSA)")
  p.add_argument("--without-external-ssh", action="store_true",
...
def ensure_permissions(path: str, mode: int, uid: int, gid: int, is_dir=False):
  ...
  try:
    ...
    print(f"Warning: could not chown {path} to uid:{uid} gid:{gid}.", file=sys.stderr)
...
def append_key_line(authorized_keys: str, line: str):
  ...
  if not (line.startswith("ssh-") or line.startswith("ecdsa-") or line.startswith("sk-") or " " in line):
    print("Warning: provided SSH key doesn't look like a public key.", file=sys.stderr)
...
def main():
  ...
  try:
    ...
    print(f"Error: passwd entry for '{args.username}' not found after adduser.", file=sys.stderr)
    sys.exit(1)
  ...
  if not args.without_internal_ssh:
    ...
    run([ssh_keygen, "-t", "ecdsa", "-f", internal_key, "-N", "", "-C", f"{args.username}@soperator-internal"])
    for p in (internal_key, internal_key + ".pub"):
      try:
        os.chown(p, uid, gid)
      except PermissionError:
        print(f"Warning: could not chown {p} to {args.username}.", file=sys.stderr)
    try:
      with open(internal_key + ".pub", "r", encoding="utf-8") as pubf:
        append_key_line(authorized_keys, pubf.read().strip())
    except FileNotFoundError:
      print("Warning: internal public key not found; skipping append.", file=sys.stderr)
  ...
  if added_groups:
    ...
    print(f"User '{args.username}' created. Home: {home_dir}. Added to groups: {groups_str}.")
  else:
