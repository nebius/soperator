...
const prepullActiveCheckBaseName = "prepull-container-image"
...
func PrepullContainerImagePyxis(ctx context.Context, exec framework.Exec, clusterName string) (string, error) {
	...
	for _, c := range checks.Items {
		...
		if script == "" {
			return "", fmt.Errorf("ActiveCheck %s has empty sbatchScript", checkName)
		}
		m := containerImageRegex.FindStringSubmatch(script)
		if len(m) < 2 {
			return "", fmt.Errorf("ActiveCheck %s sbatchScript has no --container-image= argument", checkName)
		}
		image := strings.TrimPrefix(strings.TrimSpace(m[1]), "docker://")
		if image == "" {
			return "", fmt.Errorf("ActiveCheck %s --container-image= is empty", checkName)
		}
		return image, nil
	}
	return "", fmt.Errorf("ActiveCheck %s not found", checkName)
}

// PrepullContainerImageDocker returns the same image in plain Docker URL form
// ("reg/repo:tag"), which is what `docker run` accepts.
func PrepullContainerImageDocker(ctx context.Context, exec framework.Exec, clusterName string) (string, error) {
	pyxis, err := PrepullContainerImagePyxis(ctx, exec, clusterName)
	if err != nil {
		return "", err
	}
...
func PrepullContainerImageEnroot(ctx context.Context, exec framework.Exec, clusterName string) (string, error) {
	pyxis, err := PrepullContainerImagePyxis(ctx, exec, clusterName)
	if err != nil {
		return "", err
	}
