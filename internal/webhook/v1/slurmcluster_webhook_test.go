...
func TestValidateSlurmClusterPAMSlurmAdopt(t *testing.T) {
	validator := &SlurmClusterCustomValidator{}
	clusterWith := func(config *slurmv1.PAMSlurmAdopt) *slurmv1.SlurmCluster {
		return &slurmv1.SlurmCluster{
			Spec: slurmv1.SlurmClusterSpec{PAMSlurmAdopt: config},
		}
	}

	t.Run("admits omitted configuration", func(t *testing.T) {
		_, err := validator.ValidateCreate(context.Background(), clusterWith(nil))
		assert.NoError(t, err)
	})

	t.Run("admits empty exemptions", func(t *testing.T) {
		_, err := validator.ValidateCreate(context.Background(), clusterWith(&slurmv1.PAMSlurmAdopt{}))
		assert.NoError(t, err)
	})

	t.Run("admits local and SSSD identities", func(t *testing.T) {
		_, err := validator.ValidateCreate(context.Background(), clusterWith(&slurmv1.PAMSlurmAdopt{
			ExemptUsers:  []string{"service-user", "admin@example.com", `EXAMPLE\admin`, "machine$"},
			ExemptGroups: []string{"cluster_admins", "platform operators"},
		}))
		assert.NoError(t, err)
	})
	...
	for _, test := range []struct {
		...
		{
			name:   "rejects empty user",
			config: &slurmv1.PAMSlurmAdopt{ExemptUsers: []string{""}},
		},
		{
			name:   "rejects user containing newline",
			config: &slurmv1.PAMSlurmAdopt{ExemptUsers: []string{"admin\nroot"}},
		},
		{
			name:   "rejects group with surrounding whitespace",
			config: &slurmv1.PAMSlurmAdopt{ExemptGroups: []string{" admins "}},
		},
		{
			name:   "rejects duplicate user",
			config: &slurmv1.PAMSlurmAdopt{ExemptUsers: []string{"admin", "admin"}},
		},
		{
			name:   "rejects duplicate group",
			config: &slurmv1.PAMSlurmAdopt{ExemptGroups: []string{"admins", "admins"}},
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			_, err := validator.ValidateCreate(context.Background(), clusterWith(test.config))
			assert.Error(t, err)
		})
	}

	t.Run("admits required parameters in raw overrides", func(t *testing.T) {
		customConfig := "PrologFlags=Alloc,Contain\nLaunchParameters=use_interactive_step,ulimit_pam_adopt"
		cluster := clusterWith(&slurmv1.PAMSlurmAdopt{Enabled: ptr.To(true)})
		cluster.Spec.CustomSlurmConfig = &customConfig
	...
	for _, test := range []struct {
		...
		{
			name:         "rejects PrologFlags override without contain",
			customConfig: "PrologFlags=Alloc",
			errorText:    "contain",
		},
		{
			name:         "rejects LaunchParameters override without ulimit_pam_adopt",
			customConfig: "LaunchParameters=use_interactive_step",
			errorText:    "ulimit_pam_adopt",
		},
		{
			name:         "validates the effective last override",
			customConfig: "PrologFlags=Contain\nPrologFlags=Alloc",
			errorText:    "contain",
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			cluster := clusterWith(&slurmv1.PAMSlurmAdopt{Enabled: ptr.To(true)})
			cluster.Spec.CustomSlurmConfig = &test.customConfig
	...
	t.Run("ignores raw overrides while disabled", func(t *testing.T) {
		customConfig := "PrologFlags=Alloc\nLaunchParameters=use_interactive_step"
		cluster := clusterWith(&slurmv1.PAMSlurmAdopt{Enabled: ptr.To(false)})
		cluster.Spec.CustomSlurmConfig = &customConfig
