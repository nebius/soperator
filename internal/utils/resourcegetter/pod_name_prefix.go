package resourcegetter

import slurmv1 "nebius.ai/slurm-operator/api/v1"

// ResolvePodNamePrefix returns the prefix to use for pod workload resource names.
// An empty mode is treated as enabled for objects created before the field was introduced.
func ResolvePodNamePrefix(clusterName string, mode slurmv1.PodNamePrefixMode) string {
	if mode == slurmv1.PodNamePrefixDisabled {
		return ""
	}

	return clusterName
}

func BuildPrefixedName(prefix, base string) string {
	if prefix == "" {
		return base
	}
	return prefix + "-" + base
}
