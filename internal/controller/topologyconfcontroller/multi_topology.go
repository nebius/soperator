...
func (r *WorkerTopologyReconciler) buildMultiTopologyYAML(
	...
	// CPU-only NodeSets are described by the generated flat topology, never by a user-defined one:
	// they sit on no fabric, and hanging them off a switch inside an IB tree would tell the
	// scheduler they are one hop from the GPU nodes.
	gpuNodeSets := topologyrefs.GPUNodeSets(nodeSetList)
	cpuNodeSets := topologyrefs.CPUOnlyNodeSets(nodeSetList)

	specs := namedTopologies(slurmCluster)
	entries := make([]topologyYAMLEntry, 0, len(specs)+1)
	for _, spec := range specs {
		nodeSets := selectNodeSets(gpuNodeSets, spec.NodeSetRefs)
		if len(nodeSets) == 0 {
	...
	entries = appendCPUOnlyTopology(ctx, entries, specs, cpuNodeSets)
	markClusterDefault(ctx, entries)

	return renderTopologyYAML(entries)
}

// appendCPUOnlyTopology adds the generated flat topology covering every CPU-only NodeSet.
//
// It goes last and is never flagged as the cluster default: Slurm uses the first entry of
// topology.yaml for operations not tied to a partition, such as slurmctld-to-slurmd message
// forwarding, and those should keep following a real fabric wherever the cluster has one. A cluster
// with no GPU NodeSets at all is the exception that proves it — every user topology drops out for
// lack of nodes, and the flat one is left to become the default on its own.
func appendCPUOnlyTopology(
	ctx context.Context,
	entries []topologyYAMLEntry,
	specs []slurmv1.NamedTopology,
	cpuNodeSets []v1alpha1.NodeSet,
) []topologyYAMLEntry {
	if len(cpuNodeSets) == 0 {
		return entries
	}

	logger := log.FromContext(ctx).WithName(WorkerTopologyReconcilerName)
	for _, spec := range specs {
		if spec.Name == consts.SlurmTopologyCPUOnlyName {
			logger.Info("A configured topology already uses the CPU-only topology name, "+
				"not generating one; CPU-only NodeSets will follow it instead",
				"topology", consts.SlurmTopologyCPUOnlyName)
			return entries
		}
	}

	return append(entries, topologyYAMLEntry{
		Topology: consts.SlurmTopologyCPUOnlyName,
		Flat:     true,
	})
}
