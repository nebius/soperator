...
`,
		...
		{
			// The provisioning-time layout: a flat topology covering everything, an IB tree and an
			// NVL block over the GPU NodeSets. Every topology is declared with ALL, and the
			// CPU-only NodeSet still reaches only the flat one.
			name: "flat covers everything while tree and block list GPU NodeSets only",
...
`,
		...
		{
			name: "a topology matching no NodeSet is rendered empty, not left out",
			cluster: clusterWithTopologies(
				slurmv1.NamedTopology{
					Name:           "present",
					ClusterDefault: ptr.To(true),
			...
			// Emitted rather than skipped: a partition bound to a topology missing from the file
			// makes slurmctld reject the whole config. An empty block list would be fatal too, so
			// it carries one named, empty block.
...
// TestBuildMultiTopologyYAML_ZeroReplicas pins that a NodeSet scaled to zero is treated as reaching
// no node. Building from it would emit "blocks: []" without block_sizes, which makes slurmctld fatal
// with "Blocks do not contain any nodes and the BlockSizes are not set".
func TestBuildMultiTopologyYAML_ZeroReplicas(t *testing.T) {
	r := &WorkerTopologyReconciler{}

	scaledToZero := gpuNodeSet("gb300", 0, "")

	cluster := clusterWithTopologies(
		slurmv1.NamedTopology{
			Name:        "tree-ib",
			Topo:        slurmv1.TopologyPlugin{Type: consts.SlurmTopologyTypeTree},
			NodeSetRefs: []string{"gb300"},
		},
		slurmv1.NamedTopology{
			Name:        "block-nvl72",
			Topo:        slurmv1.TopologyPlugin{Type: consts.SlurmTopologyTypeBlock},
	...
	out, err := r.buildMultiTopologyYAML(
		...
		[]v1alpha1.NodeSet{scaledToZero}, nodeLabelsCM(nil), nil,
	)
	require.NoError(t, err)

	assert.NotContains(t, out, "blocks: []")
	assert.NotContains(t, out, "switches: []")
	assert.Contains(t, out, "block_sizes:")
}
