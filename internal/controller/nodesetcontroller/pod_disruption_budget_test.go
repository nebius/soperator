...
func TestReconcileWorkerDisruptionBudgetLifecycle(t *testing.T) {
	...
	r := NewNodeSetReconciler(kubeClient, scheme, record.NewFakeRecorder(10), false)
	...
	require.True(t, apierrors.IsNotFound(r.Get(ctx, key, pdb)))

	r = NewNodeSetReconciler(kubeClient, scheme, record.NewFakeRecorder(10), true)
	require.NoError(t, r.reconcilePodDisruptionBudget(ctx, owner, nodeSet))
	...
	r.rollingUpdateEnabled = false
	require.NoError(t, r.reconcilePodDisruptionBudget(ctx, owner, nodeSet))
	assert.True(t, apierrors.IsNotFound(r.Get(ctx, key, pdb)))
	require.NoError(t, r.reconcilePodDisruptionBudget(ctx, owner, nodeSet))
	assert.True(t, apierrors.IsNotFound(r.Get(ctx, key, pdb)))

	r.rollingUpdateEnabled = true
	require.NoError(t, r.reconcilePodDisruptionBudget(ctx, owner, nodeSet))
	require.NoError(t, r.Get(ctx, key, pdb))
