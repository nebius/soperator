...
type NodeConfiguratorReconciler struct {
	...
	// feature for the DaemonSet generated by this controller.
...
func (r *NodeConfiguratorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	r.applyResourcePatchPolicies(ctx, nodeConfigurator, desired)
...
// ResourcePatchPolicy that targets the given NodeConfigurator. It is a no-op
// unless the feature is enabled.
func (r *NodeConfiguratorReconciler) applyResourcePatchPolicies(
	ctx context.Context,
	owner *slurmv1alpha1.NodeConfigurator,
	desired client.Object,
) {
	...
	matching := resourcepatch.FilterPoliciesForTarget(
		list.Items,
		slurmv1alpha1.GroupVersion.Group,
		slurmv1alpha1.KindNodeConfigurator,
		owner.GetName(),
		owner.GetNamespace(),
	)
	if len(matching) == 0 {
	...
	if err != nil {
		logger.Error(err, "Failed to apply ResourcePatchPolicy")
		return
	}
...
func (r *NodeConfiguratorReconciler) SetupWithManager(mgr ctrl.Manager, maxConcurrency int, cacheSyncTimeout time.Duration) error {
	...
	if r.EnableResourcePatchPolicy {
		b = b.Watches(
			...
			handler.EnqueueRequestsFromMapFunc(resourcepatch.MapPolicyToTarget(slurmv1alpha1.KindNodeConfigurator)),
		)
	}
