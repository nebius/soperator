...
func Test_RenderSecret(t *testing.T) {
	...
	secret, err := accounting.RenderSecret(defaultNamespace, defaultNameCluster, acc, defaultSecret)
...
func Test_RenderSecret_Errors(t *testing.T) {
	...
	_, err := accounting.RenderSecret(defaultNamespace, defaultNameCluster, nil, nil)
	assert.Equal(t, accounting.ErrSecretNil, err.Error())

	// // Test with empty secret data
	testSecret := &corev1.Secret{}
	_, err = accounting.RenderSecret(defaultNamespace, defaultNameCluster, acc, testSecret)
