...
func generateDefaultSupervisordConfig() renderutils.ConfigFile {
	...
	res.AddLine("stderr_logfile_maxbytes=0")
	res.AddLine("redirect_stderr=true")
	res.AddLine("command=/opt/bin/slurm/dockerd_entrypoint.sh chroot /mnt/jail /usr/bin/dockerd")
	res.AddLine("autostart=true")
	res.AddLine("autorestart=unexpected")
	res.AddLine("exitcodes=0")
	res.AddLine("startsecs=0")
	res.AddLine("stopasgroup=true")
	res.AddLine("killasgroup=true")
	res.AddLine("stopsignal=SIGTERM")
	res.AddLine("stopwaitsecs=10")
	res.AddLine("")
	res.AddLine("[program:docker-proxy]")
	res.AddLine("priority=2")
	res.AddLine("stdout_logfile=/dev/fd/1")
	res.AddLine("stdout_logfile_maxbytes=0")
	res.AddLine("stderr_logfile=/dev/fd/2")
	res.AddLine("stderr_logfile_maxbytes=0")
	res.AddLine("redirect_stderr=true")
	res.AddLine("command=/opt/bin/slurm/docker_proxy_entrypoint.sh worker")
	res.AddLine("autostart=true")
	res.AddLine("autorestart=unexpected")
	res.AddLine("exitcodes=0")
	res.AddLine("startsecs=0")
