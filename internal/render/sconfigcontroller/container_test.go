...
func TestRenderContainerSConfigController(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "Configuration with timeout parameters",
			clusterNamespace: "timeout-namespace",
			...
			sConfigController: &values.SConfigController{
				Container: values.Container{
					...
					NodeContainer: slurmv1.NodeContainer{
						Image:           "timeout-image:v2",
						ImagePullPolicy: corev1.PullIfNotPresent,
						Resources:       corev1.ResourceList{},
					},
				},
				ReconfigurePollInterval: ptr.To("30s"),
				ReconfigureWaitTimeout:  ptr.To("2m"),
			},
			expectedCommand: []string{"/usr/bin/sconfigcontroller"},
			expectedArgs: []string{
				"--cluster-namespace=timeout-namespace",
			...
			expectedImage: "timeout-image:v2",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.ImagePullPolicy != tt.sConfigController.Container.ImagePullPolicy {
				t.Errorf("expected image pull policy %s, got %s", tt.sConfigController.Container.ImagePullPolicy, result.ImagePullPolicy)
			}
