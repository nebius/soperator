	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedTopologyPlugin == "" {
				assert.NotContains(t, result.Data[consts.ConfigMapKeyAutogenSlurmConfig], "TopologyPlugin")
			} else {
				assert.Contains(t, result.Data[consts.ConfigMapKeyAutogenSlurmConfig], "TopologyPlugin="+tt.expectedTopologyPlugin)
			}

			if tt.unexpectedTopologyPlugin != "" {
				assert.NotContains(t, result.Data[consts.ConfigMapKeyAutogenSlurmConfig], "TopologyPlugin="+tt.unexpectedTopologyPlugin)
			}
		})
	}
}
...
func TestRenderSlurmConfigMapWithLargeSuspendTime(t *testing.T) {
	...
	slurmConfig := result.Data[consts.ConfigMapKeyAutogenSlurmConfig]
...
func TestRenderConfigMapSlurmConfigs_FileNamesAndWarnings(t *testing.T) {
	...
	assert.Contains(t, result.Data, consts.ConfigMapKeyAutogenSlurmConfig)
	...
	assert.Contains(t, result.Data[consts.ConfigMapKeySlurmConfig], "include "+slurmConfigPath(consts.ConfigMapKeyAutogenSlurmConfig))
	assert.Contains(t, result.Data[consts.ConfigMapKeyAutogenSlurmConfig], ManagedSlurmConfigWarning())
	assert.Contains(t, result.Data[consts.ConfigMapKeyAutogenSlurmConfig], "include "+slurmConfigPath(consts.ConfigMapKeyCustomSlurmConfig))
	...
	warningText := strings.Join([]string{
		...
		result.Data[consts.ConfigMapKeyAutogenSlurmConfig],
...
func TestRenderSlurmConfig_MetricsType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			conf := result.Data[consts.ConfigMapKeyAutogenSlurmConfig]
			if tt.expectLine {
