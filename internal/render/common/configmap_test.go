...
func TestRenderPlugstack(t *testing.T) {
	t.Run("Pyxis no options", func(t *testing.T) {
		...
		assert.Contains(t, result, "optional spank_pyxis.so runtime_path=/run/pyxis execute_entrypoint=0 container_scope=global sbatch_support=1")
		assert.NotContains(t, result, "use_squashfuse=1")
		assert.NotContains(t, result, "importer=")
		assert.NotContains(t, result, "container_image_save=")
	})

	t.Run("Pyxis importer options", func(t *testing.T) {
		result := generateSpankConfig(&values.SlurmCluster{
			PlugStackConfig: slurmv1.PlugStackConfig{
				Pyxis: slurmv1.PluginConfigPyxis{
	...
	t.Run("Pyxis deprecated container_image_save fallback", func(t *testing.T) {
		result := generateSpankConfig(&values.SlurmCluster{
			PlugStackConfig: slurmv1.PlugStackConfig{
				Pyxis: slurmv1.PluginConfigPyxis{
					Required:           ptr.To(true),
		...
		assert.NotEmpty(t, result)
		assert.Contains(t, result, "required spank_pyxis.so runtime_path=/run/pyxis execute_entrypoint=0 container_scope=global sbatch_support=1 container_image_save=/var/cache/enroot-container-images/")
		assert.NotContains(t, result, "use_squashfuse=1")
		assert.NotContains(t, result, "importer=")
	})
