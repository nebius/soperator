...
func TestRenderPlugstack(t *testing.T) {
	...
	t.Run("NCCL no options", func(t *testing.T) {
		...
		assert.NotEmpty(t, result)
		assert.Contains(t, result, "optional spanknccldebug.so enabled=0 log-level=INFO out-file=0 out-dir=/var/spool/slurmd/soperator-outputs/%h/nccl_logs out-stdout=0")
	})
