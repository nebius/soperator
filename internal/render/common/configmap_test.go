	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedTopologyPlugin == "" {
				assert.NotContains(t, result.Data[consts.ConfigMapKeySlurmGenConfig], "TopologyPlugin")
			} else {
				assert.Contains(t, result.Data[consts.ConfigMapKeySlurmGenConfig], "TopologyPlugin="+tt.expectedTopologyPlugin)
			}

			if tt.unexpectedTopologyPlugin != "" {
				assert.NotContains(t, result.Data[consts.ConfigMapKeySlurmGenConfig], "TopologyPlugin="+tt.unexpectedTopologyPlugin)
			}
		})
	}
}
...
func TestRenderSlurmConfigMapWithLargeSuspendTime(t *testing.T) {
	...
	slurmConfig := result.Data[consts.ConfigMapKeySlurmGenConfig]
...
func TestRenderConfigMapSlurmConfigs_FileNamesAndWarnings(t *testing.T) {
	...
	assert.Contains(t, result.Data, consts.ConfigMapKeySlurmGenConfig)
	assert.Contains(t, result.Data, consts.ConfigMapKeyCustomSlurmConfig)
	assert.NotContains(t, result.Data, "custom_slurm.conf")

	assert.Contains(t, result.Data[consts.ConfigMapKeySlurmConfig], SlurmConfigEntrypointWarning())
	assert.Contains(t, result.Data[consts.ConfigMapKeySlurmConfig], "include "+slurmConfigPath(consts.ConfigMapKeySlurmGenConfig))
	assert.Contains(t, result.Data[consts.ConfigMapKeySlurmConfig], "include "+slurmConfigPath(consts.ConfigMapKeyCustomSlurmConfig))
	assert.Contains(t, result.Data[consts.ConfigMapKeySlurmGenConfig], ManagedSlurmConfigWarning())
	assert.NotContains(t, result.Data[consts.ConfigMapKeySlurmGenConfig], "include "+slurmConfigPath(consts.ConfigMapKeyCustomSlurmConfig))
	assert.Contains(t, result.Data[consts.ConfigMapKeyCustomSlurmConfig], "Overrides are generated by Soperator.")
	...
	warningText := strings.Join([]string{
		...
		result.Data[consts.ConfigMapKeySlurmGenConfig],
		result.Data[consts.ConfigMapKeyCustomSlurmConfig],
	}, "\n")
...
func TestRenderSlurmConfig_MetricsType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			conf := result.Data[consts.ConfigMapKeySlurmGenConfig]
			if tt.expectLine {
