...
// buildNodeSetNodeNamesMap creates a map of NodeSet names to their generated node names
func buildNodeSetNodeNamesMap(nodeSets []slurmv1alpha1.NodeSet) map[string]string {
	nodeSetMap := make(map[string]string)
...
// addStructuredPartitions adds structured partition configuration to the slurm config
func addStructuredPartitions(res *renderutils.PropertiesConfig, cluster *values.SlurmCluster) {
	res.AddComment("# Structured partitions will be generated by partition config reconciler")
	...
	nodeSetNodeNamesMap := buildNodeSetNodeNamesMap(cluster.NodeSetList.Items)

	allNodeNamesList := make([]string, 0, len(nodeSetNodeNamesMap))
	var allPartition slurmv1.Partition
	...
	for _, partition := range cluster.PartitionConfiguration.Partitions {
		...
		if len(partition.NodeSetRefs) > 0 {
			...
			for _, nodeSetRef := range partition.NodeSetRefs {
				if nodeNames, exists := nodeSetNodeNamesMap[nodeSetRef]; exists {
					nodeNamesList = append(nodeNamesList, nodeNames)
					allNodeNamesList = append(allNodeNamesList, nodeNames)
				}
			}
			if len(nodeNamesList) > 0 {
				nodesStr := strings.Join(nodeNamesList, ",")
				config := fmt.Sprintf("%s Nodes=%s", partition.Name, nodesStr)
				if partition.Config != "" {
					config = fmt.Sprintf("%s %s", config, partition.Config)
				}
				res.AddProperty("PartitionName", config)
			}
		}
	}
	if allPartition.Name != "" && len(allNodeNamesList) > 0 {
		allNodesStr := strings.Join(allNodeNamesList, ",")
		config := fmt.Sprintf("%s Nodes=%s", allPartition.Name, allNodesStr)
		if allPartition.Config != "" {
			config = fmt.Sprintf("%s %s", config, allPartition.Config)
		}
		res.AddProperty("PartitionName", config)
	}
}
...
func generateSlurmConfig(cluster *values.SlurmCluster) renderutils.ConfigFile {
	...
	case "structured":
		addStructuredPartitions(res, cluster)
