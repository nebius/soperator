...
func TestStandaloneTokenIssuer_Constants(t *testing.T) {
	if DefaultScontrolPath != "scontrol" {
		t.Errorf("Expected DefaultScontrolPath 'scontrol', got %s", DefaultScontrolPath)
	}

	if DefaultRotationInterval != 30*time.Minute {
		t.Errorf("Expected DefaultRotationInterval 30m, got %v", DefaultRotationInterval)
	}

	if DefaultUsername != "root" {
		t.Errorf("Expected DefaultUsername 'root', got %s", DefaultUsername)
	}
}

func TestStandaloneTokenIssuer_GetMethods(t *testing.T) {
	clusterID := types.NamespacedName{Namespace: "test", Name: "cluster"}
	username := "testuser"
	...
	if issuer.GetClusterID() != clusterID {
		t.Errorf("GetClusterID returned %v, expected %v", issuer.GetClusterID(), clusterID)
	}

	if issuer.GetUsername() != username {
		t.Errorf("GetUsername returned %s, expected %s", issuer.GetUsername(), username)
	}

	if issuer.GetScontrolPath() != DefaultScontrolPath {
		t.Errorf("GetScontrolPath returned %s, expected %s", issuer.GetScontrolPath(), DefaultScontrolPath)
	}

	if issuer.GetRotationInterval() != DefaultRotationInterval {
		t.Errorf("GetRotationInterval returned %v, expected %v", issuer.GetRotationInterval(), DefaultRotationInterval)
	}
}

func TestStandaloneTokenIssuer_Chaining(t *testing.T) {
	clusterID := types.NamespacedName{Namespace: "test", Name: "cluster"}

	issuer := NewStandaloneTokenIssuer(clusterID, "")
	issuer = issuer.WithScontrolPath("/custom/path")
	issuer = issuer.WithRotationInterval(15 * time.Minute)

	if issuer.GetScontrolPath() != "/custom/path" {
		t.Errorf("Expected custom scontrol path, got %s", issuer.GetScontrolPath())
	}

	if issuer.GetRotationInterval() != 15*time.Minute {
		t.Errorf("Expected custom rotation interval, got %v", issuer.GetRotationInterval())
	}
}
