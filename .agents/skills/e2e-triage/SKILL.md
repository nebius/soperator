...
```
.e2e-triage/{run_id}/
├── run.json              — metadata, steps, profile, o11y IDs
├── logs/                 — per-step log files (NN-step-name.log) and resolve-profile.log
└── artifacts/            — pre-downloaded and extracted artifacts (cluster-info/, jail*/, etc.)
```

Step log files referenced in `run.json` `steps[].file` are inside the `logs/` subdirectory.
...
2. Read the last ~200 lines of the failed step's log file in `logs/` (errors are at the bottom).
...
- When a pod is central to the failure (e.g. wait-for-soperatorchecks-*, wait-for-active-checks-*, controller-0), always get its logs. If the pod's logs are not in the cluster-info artifact (common when pods are destroyed by Flux uninstall remediation), immediately query Grafana via `npc logging` — do NOT stop at secondary evidence like sacct exit codes or DNS errors. For controller-0 CrashLoopBackOff, only the first run shows the real error (later restarts log irrelevant messages like "fatal: No Assoc usage file"), so use `--forward` and `--limit` to get the earliest logs.
  The o11y project and workspace IDs are in `run.json` (under `o11y.project_id` and `o11y.workspace_id`). Query example:
  ```
  npc logging query '{__workspace__="<o11y.workspace_id>", __bucket__="default", k8s_pod_name=~"<prefix>.*"}' --project-id <o11y.project_id> --since <run_started_at> --until <updated_at> --limit 100 --forward
  ```
  - Use `--forward` for oldest-first order, and `|= "error"` or `|= "fatal"` LogQL filters to narrow down
  - Note: `npc logging` displays timestamps in local time, but `--since`/`--until` accept UTC. Pass `run_started_at`/`updated_at` from `run.json` as-is
...
You must mention evidences (e.g. log extracts) everywhere: in plain output, Slack message, Jira comments or
Jira ticket description (if creating new ticket).

### Slack message

Reply in the Slack thread provided in Phase 1. Extract `channel_id` and `thread_ts` from the Slack thread URL (format: `https://nebius.slack.com/archives/<channel_id>/p<thread_ts_without_dot>`; convert `p1773949701936079` → `1773949701.936079`).

Format the message using standard markdown: `**bold**` for labels, `` `code` `` for snippets, `[text](url)` for links. Jira tickets must be hyperlinks (e.g. `[SCHED-1184](https://nebius.atlassian.net/browse/SCHED-1184)`). Do NOT include the Slack thread URL in the message — it will be posted into that same thread. Include the profile name in the message.
...
- Option 1: "Create a new Jira ticket with `soperator-e2e-fail`, `tech-debt`, `dataPlane` labels"
