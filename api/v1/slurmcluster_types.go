...
type Topology struct {
	...
	//
	// Deprecated: superseded by Topologies, which describes the same thing per topology and
	// supports heterogeneous fabrics. BlockSize is honoured only while Topologies is empty and
	...
	// Topologies defines several named network topologies rendered into topology.yaml, letting a
	...
	//
	// Leaving it empty keeps the legacy single-topology behaviour driven by
	// SlurmConfig.TopologyPlugin and BlockSize, which renders topology.conf instead.
	//
	// +kubebuilder:validation:Optional
...
type NamedTopology struct {
	...
	//
	// +kubebuilder:validation:Required
	Topo TopologyPluginSpec `json:"topo"`
...
// TopologyPluginSpec is a discriminated union: Type picks the plugin, and the remaining fields
// apply only to the matching plugin.
type TopologyPluginSpec struct {
	// Type identifies the topology plugin backing this topology.
	//
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:Enum=tree;block
