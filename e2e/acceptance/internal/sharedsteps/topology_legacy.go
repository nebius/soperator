...
// TopologyLegacy covers clusters older than 5.0.0, where Slurm has a single topology/tree
// configuration instead of the named topologies of topology.yaml. See Topology for the current one.
type TopologyLegacy struct {
...
func NewTopologyLegacy(runtime framework.Runtime, selector *framework.WorkerSelector) *TopologyLegacy {
	return &TopologyLegacy{runtime: runtime, selector: selector}
}

func (s *TopologyLegacy) RegisterSteps(sc *godog.ScenarioContext) {
...
func (s *TopologyLegacy) CleanupAndReset(ctx context.Context) {
	s.workers = nil
	s.graph = nil
	s.reportedAddrs = nil
}

func (s *TopologyLegacy) theSlurmTopologyPluginIsTree(ctx context.Context) error {
...
func (s *TopologyLegacy) scontrolTopologyIsParsedIntoASwitchTree(ctx context.Context) error {
...
func (s *TopologyLegacy) everyWorkerIsPresentInTheTopology() error {
	if s.graph == nil {
...
func (s *TopologyLegacy) aJobRunsOnAllAvailableWorkers(ctx context.Context) error {
...
func (s *TopologyLegacy) eachTaskAddrMatchesTheTree() error {
	if s.graph == nil {
		return fmt.Errorf("topology graph not parsed yet")
	}
