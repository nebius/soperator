...
![Soperator overview](docs/images/hero.png)
...
|---|---|
| **Slow cluster setup and painful day-2 ops.** Standing up Slurm takes weeks of glue code. Resizing, upgrading or reconfiguring a live cluster is just as painful. Keeping the software stack identical on every node is a constant source of drift. | Declarative cluster management through a single `SlurmCluster` CR, plus a shared root filesystem (the *jail*), reduce setup time and eliminate configuration drift across nodes. |
...
Organized around the three pillars above. Everything listed here is in the codebase today.
...
![Soperator architecture](docs/images/architecture.png)
...
|---|---|---|
| **Self-deploy on any Kubernetes** | Teams running their own K8s, on any cloud or on-prem | [Guide](https://docs.nebius.com/slurm-soperator/deploy/overview#self-deployment-on-other-platforms-and-on-premises) |
| **Self-deploy on Nebius (Terraform)** | Teams standing up a greenfield cluster on Nebius | [Guide](https://docs.nebius.com/slurm-soperator/deploy/overview#self-deployment-in-nebius-ai-cloud) |
| **Managed Soperator on Nebius** | Teams who want a training-ready cluster and only want to pay for compute | [Get started](https://nebius.com/services/soperator) |
